...
export default class LeaveEnterTransition extends StageDispatcher {
  ...
  constructor(
    ...
    name,
    {
      css = true,
      duration = null,
      stageIndex = LeaveEnterTransition.STAGE_LEAVE_INDEX,
      events = true,
    ...
    // LeaveStage.use(LeaveGroupMiddleware)
    // LeaveStage.use(new CSSMiddleware(element, {fromClass, activeClass, toClass, doneClass}))
    // LeaveStage.use(new AutoRectEnterMiddleware(element, `${this.name}-leave`, {auto}))
    // LeaveStage.use(new HideAfterEndMiddleware(element))
    // LeaveStage.use(new EventsMiddleware(element))
    super(
      element,
      name,
      [
