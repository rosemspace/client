export const doctypeDeclarationRegExp = /^\s*<!DOCTYPE ([^>]+)>/i
...
export function isAnyRawTextElement(tagName: string): boolean {
  return rawTextElementRegExp.test(tagName) || escapableRawTextElementRegExp.test(tagName)
}
...
// export const conditionalCommentRegExp = /^<!(\[[\s\S]*?]>[\s\S]*?<!\[[\s\S]*?])>/
